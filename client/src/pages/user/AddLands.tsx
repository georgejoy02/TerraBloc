...
const AddLands: React.FC = () => {
  ...
  const { area, address, landPrice, pid, surveyNo } = useSelector(
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.log(account);
      if (landContract) {
        const result = await axios.post("http://localhost:4000/islandreg", {
          pid: pid,
        });
        if (result.data === true) {
        ...
        if (data && data.length >= 4) {
          ...
          if (docUrl) {
            ...
            const test = await landContract.methods
              .addLand(area, address, landPrice, plot, pid, surveyNo, docUrl)
              .send({ from: account });
