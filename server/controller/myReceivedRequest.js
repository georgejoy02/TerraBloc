...
const myReceivedRequest = async (req, res) => {
    try {
        const { key } = req.body;
        console.log("mySentRequest-key: ", key);
        const land_contract = await getWeb3();
        const result = await land_contract.methods.myReceivedLandRequests(key).call();
        console.log("myReceivedLandRequests: ", result)
        const array = [];
...
module.exports = myReceivedRequest